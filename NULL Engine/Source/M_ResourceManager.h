...
public:																								// --- RESOURCE MANAGER API ---
	...
	bool			GetForcedUIDsFromMeta(const char* assetsPath, std::map<std::string, uint32>& forcedUIDs);						// Gets the UIDs stored in a .meta and assets path.
...
private:																														// --- META FILE METHODS ---
	...
	ParsonNode		LoadMetaFile(const char* assetsPath, char** buffer);														// Passing the buffer so it can be safely RELEASED after call.
