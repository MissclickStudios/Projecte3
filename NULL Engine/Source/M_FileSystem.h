...
enum class PhysfsResult																					// Every PhysFS method that rets 1/0 upon success/failure will be cast to this type.
{
	FAILURE,
	SUCCESS
};
...
public:																											// --- UTILITY METHODS
	...
	std::string		GetPathRelativeToAssets(const char* originalPath) const;									// Will get the path relative to the assets from the given path.
	...
	std::string		NormalizePath(const char* fullPath) const;													// Normalizes the dashes in the given path from '\\' to '/'.
